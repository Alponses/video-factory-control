...
export function ScheduleControls({ publication, onReload, onDirtyChange }: Props) {
  ...
  const load = async () => {
    ...
    try {
      ...
      setPreflight(result);
      if (!dirty && result.activeSchedule?.localDateTime) {
  ...
  const canSubmit = Boolean(localDateTime && timezone && ready && !busy);
  ...
  return <section className="schedule-card" aria-label={`${publication.platform} scheduling`}>
    <div className="section-toolbar"><div><h3>Schedule</h3><p className="muted">UTC persistence · IANA timezone · dispatch snapshot at due time.</p></div><span className={ready ? 'preflight-ready' : 'preflight-blocked'}>{ready ? 'Preflight ready' : 'Preflight blocked'}</span></div>
    ...
    {blockers.length > 0 ? <div className="warning-banner" role="alert"><strong>Blockers</strong><ul>{blockers.map((item) => <li key={item.code}>{item.code}: {item.message}</li>)}</ul></div> : null}
    {warnings.length > 0 ? <div className="info-banner"><strong>Warnings</strong><ul>{warnings.map((item) => <li key={item.code}>{item.message}</li>)}</ul></div> : null}
